...
def test_apply():
    ...
    for pdb, sym in zip(("2CC9", "1STM", "7NTN"), ("T", "O", "I")):
        # SETUP
        ...
        for norm, ref in zip((cs_hf_norm, cs_3_norm, cs_2_norm),  (cs_hf, cs_3, cs_2)):
            ...
            sa.apply()
            assert math.isclose(sa.al_from.CA_rmsd_hf_map(sa.pose_from, pose_original_symmetrized), sa.final_rmsd, abs_tol=0.1)
            assert math.isclose(sa.al_from.CA_rmsd_hf_map(sa.pose_from, pose_crystal), sa.final_rmsd, abs_tol=0.1)
        ...
        for pdb in others[sym]:
            ...
            for base, ref in zip(("HF", "3F", "2F"), (sds.foldHF_setup, sds.fold3_setup, sds.fold2_setup)):
                # a)
                ...
                if same_handedness is None and norm.is_hf_based():
                    same_handedness = norm.is_rightanded() == sds.foldHF_setup.is_rightanded()
                ...
                sa.apply()
                assert math.isclose(sa.al_from.CA_rmsd_hf_map(sa.pose_from, pose_original_symmetrized, same_handedness=same_handedness), sa.final_rmsd, abs_tol=0.1)
                assert math.isclose(sa.al_from.CA_rmsd_hf_map(sa.pose_from, pose_crystal, same_handedness=same_handedness), sa.final_rmsd, abs_tol=0.5) # 1T0T has 0.2 RMSD between the symmetrial and crystal so this should be high
    ...
    pd.DataFrame(df).to_csv("outputs/normalization_info.csv", index=False)