...
def test_boundedminmover():
    ...
    from cubicsym.dofspecificationcreator import DofspecificationCreator
    ...
    cb = CubicBoundary(symdef, pose, dofspecification=DofspecificationCreator.create_uniform_dofs(-5, 5))
    ...
    assert bmm.passed
    assert all(not math.isclose(i, j) for i, j in zip(cb.cubicdofs.get_positions_as_list(pose_mod_in),
                                                  cb.cubicdofs.get_positions_as_list(pose_mod)))
    ...
    assert all(math.isclose(i, j) for i, j in zip(cb.cubicdofs.get_positions_as_list(pose_mod_in),
                                                      cb.cubicdofs.get_positions_as_list(pose_mod)))
